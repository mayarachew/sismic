...
void i2cInit()
{   // Usa a interface B1 com os pinos P4.1 e P4.2
    UCB1CTLW0  = UCSWRST;                   // Reseta a interface
    UCB1CTLW0 |= 0x0F00 | UCSSEL__SMCLK;    // Configura como mestre
    UCB1BRW    = 100;                       // Freq = 1M / 100 = 10k
    P4SEL     |= (BIT1 | BIT2);             // Usa os pinos P4.{1,2}
    UCB1CTLW0 &= ~UCSWRST;                  // Libera o RST
}

uint8_t i2cSend(uint8_t addr, uint8_t * data, uint8_t nBytes)
{
    UCB1I2CSA = addr;                       // Configura o endereco
    UCB1CTLW0 |= UCTXSTT | UCTR;            // Transmite o endereco

    while(!(UCB1IFG & UCTXIFG));            // Espera o start acontecer
    UCB1TXBUF = * data++;                   // Escreve o primeiro byte para
    nBytes--;                               // destravar o ciclo de ACK

    // Verifica se o escravo responde (ACK) ou nao (NACK)

    while(UCB1CTLW0 & UCTXSTT);             // Aguarda ACK/NACK
    if(UCB1IFG & UCNACKIFG)                 // Se o escravo nao responder (NACK)
    {
        UCB1CTLW0 |= UCTXSTP;               // Se o escravo nao respondeu
        while(UCB1CTLW0 & UCTXSTP);         // eu peco um stop e espero o stop
        return 1;                           // ser enviado antes de retornar 1
    }

    while(nBytes--)                         // Se o escravo responder (ACK),
    {                                       // continuar a comunicacao enviando
        while(!(UCB1IFG & UCTXIFG));        // os proximos bytes ate acabarem
        UCB1TXBUF = * data++;               // os bytes acabarem
    }

    while(!(UCB1IFG & UCTXIFG));            // Espera o ultimo byte ser enviado
    UCB1CTLW0 |= UCTXSTP;                   // e eu peco um stop

    while(UCB1CTLW0 & UCTXSTP);             // Espera o stop ser enviado
